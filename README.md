...
**LLM-spend rollup (R11).** `openclaw audit spend [--by provider|model|day|session] [--since 24h] [--until now] [--json]` aggregates `prompt.response` metadata across a time window, grouping by provider, model, calendar day, or session id. Token columns include input, output, cache-read, and cache-write counts; `costUsd` is summed verbatim from the per-event `metadata.costUsd`. Default grouping is `model`. The JSON shape is published at `schemas/audit-spend.schema.json`. Same projection consumes the daily report's LLM-spend section, so totals match `audit report daily` when the windows align.
