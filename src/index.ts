    ...
    register(api) {
      ...
      import("openclaw/plugin-sdk").then(({onDiagnosticEvent}) => {
        ...
        onDiagnosticEvent((evt: Record<string, unknown>) => {
          ...
          // `usage` is not yet typed on the diagnostic-event payload by
          // the SDK; declare its shape once rather than scattering
          // `as any` (mirrors the AgentCtxExtra/MessageEvtExtra pattern
          // in hooks.ts). Cast once and read fields off the alias.
          const usage = (evt as { usage?: {
            input?: number;
            output?: number;
            cacheRead?: number;
            cacheWrite?: number;
          ...
          try {
            // No redaction needed here: this diagnostic event records only
            // provider/model and token-usage counts — it carries no prompt or
            // completion text, so the redactPromptText path doesn't apply.
            limiter.append({
          ...
          switch (action) {
            ...
            case "verify": {
              ...
              switch (result.status) {
                ...
                default: {
                  // Exhaustiveness guard: VerifyResult is a closed
                  // union, so this is unreachable today. If a new
                  // variant is added, this becomes a compile error
                  // (never) rather than a silent `undefined` return.
                  const _exhaustive: never = result;
                  return {error: `Unknown verify result status: ${(_exhaustive as {status: string}).status}`};
                }
              }
            }
            case "trees": {
      ...
      api.registerService({
        ...
        async start() {
          ...
          const {count: pending} = activeStore.countAndMaxSince(lastSeq + 1);
          if (pending > 0) {
